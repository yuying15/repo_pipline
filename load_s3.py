...
spark = SparkSession.builder \
    .appName("Load S3 Data to Snowflake") \
...
# Save each available table to Snowflake
for table_name, df in available_tables.items():
    try:
        save_to_snowflake(df, sf_options, table_name)
    except Exception as e:
        print(f"Error saving {table_name} to Snowflake: {e}")
